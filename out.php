...
$slave = new slave;
$config = new config;
$ww_htm = RD . "/theme/work_window.htm";
$content = "";
if (file_exists("$ww_htm")) {
    $content = file_get_contents($ww_htm);
}
//-----

$cl = new client;
list($login_form, $login_forms) = $cl->get_client();
$content = str_replace("{autorization}", $login_form, $content);
$cl = new client;
$content = str_replace("{login_forms}", $login_forms, $content);
$config->ident_user();
$dep = $slave->get_dep();
$dep_ud = $slave->get_dep_up();
...
$shop = new shop;
$content = str_replace("{busket}", $shop->show_busket(), $content);
$menu = new menu;
