...
require_once (RD."/lib/client_class.php"); $cl=new client;
require_once (RD."/lib/shop_class.php"); $shop=new shop;
require_once (RD."/lib/task_class.php");

$form_htm=RD."/tpl/task_message.htm";$form="";if (file_exists("$form_htm")){ $form = file_get_contents($form_htm);}
$data=date("Y-m-d");$time=date("H:i:s"); $time_end=date("H:i:s",strtotime("+15 minutes"));

$r=$odb->query_td("select * from tasks where data='$data' and time_end>='$time' and time_end<='$time_end' and is_mailed=0 order by id asc;");$k=0;
...
$r=$odb->query_td("select oc.order_id, o.* from orders_check oc left outer join orders o on o.id=oc.order_id where oc.status='1' order by oc.order_id asc;");$k=0;$m=0;
while(odbc_fetch_row($r)){$k+=1;
	...
	while(odbc_fetch_row($r1)){
		...
		if ($lid_sum=="" or $lid_sum1==""){
			...
			$r2=$odb->query_td("select * from orders_str where order_id='$order_id';");$orSumm=0;$j=0;
			while(odbc_fetch_row($r2)){$j++;
