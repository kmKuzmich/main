...
class catalogue {
	...
	function show_sto_producent($producent=0){ $odb=new odb; $slave=new slave; 
		$menu.="<select name='sto_producent' id='sto_producent' size=1 class='tec' onchange='loadStoCategory(this[this.selectedIndex].value);'><option value='0'> --- выберите производителя ---  </option>";
	...
	function show_sto_category($producent=0, $category=0){ $odb=new odb; $slave=new slave;
		$menu.="<select name='sto_category' id='sto_category' size=1 class='tec' onchange='loadStoOtype(this[this.selectedIndex].value);'><option value='0'> --- выберите категорию оборудование ---  </option>";
	...
	function show_sto_otype($category,$otype=0){ $odb=new odb; $slave=new slave;
		$menu.="<select name='sto_otype' id='sto_otype' size=1 class='tec'><option value='0'> --- выберите тип оборудование ---  </option>";
	...
	function show_range(){session_start();$dep="23"; require_once (RD."/lib/news_class.php");$news=new news; $slave=new slave; $dep=$slave->get_dep();$w=$slave->get_w();
		...
		if ($w=="add_sto_item"){
			if ($conf==""){	$form=str_replace("{range_list}",$this->create_sto_item(),$form);}
			if ($conf=="true"){$form=str_replace("{range_list}",$this->add_sto_item(),$form);}
		}
		if ($w=="edit_sto_item"){
			if ($conf==""){	$form=str_replace("{range_list}",$this->edit_sto_item($_GET["id"]),$form);}
			if ($conf=="true"){$form=str_replace("{range_list}",$this->save_sto_item(),$form);}
		}
		if ($w=="drop_sto_item"){
			if ($conf=="true"){$form=str_replace("{range_list}",$this->drop_sto_item($_GET["id"]),$form);}
		}
		if ($w==""){
			if (($_GET["manufacture"]=="" and $_GET["model"]=="" and $_GET["modification"]=="") and ($_GET["category"]=="")){	$form=str_replace("{range_list}",$this->catalogue_art_find($_POST["art"],$_POST["by_code"],$_POST["by_sklad"],$_POST["by_name"],$_POST["by_producent"]),$form);}
			if (($_GET["manufacture"]=="" and $_GET["model"]=="" and $_GET["modification"]=="") and ($_GET["category"]!="")){ $form=str_replace("{range_list}",$this->catalogue_sto_items($_POST["producent"],$_POST["category"],$_POST["otype"]),$form);}
			if ($_GET["manufacture"]!="" and $_GET["model"]!="" and $_GET["modification"]!=""){	$form=str_replace("{range_list}",$this->loadTecGroupsList($_GET["manufacture"],$_GET["model"],$_GET["modification"]),$form);}
		}
...
//		$form=str_replace("{actionBonus}",$this->showActionBonus(),$form);
	...
	function show_maslo_category_filter($category){$db=new db;$form="";if ($category==""){$category=$this->getDefaultMasloCategory();}
		...
		for ($i=1;$i<=$n;$i++){
			...
			if ($file==$category){ $form.="<a href='#maslo=$caption' onclick='setMasloCategory(\"$file\");' style='margin-left:10px; color:#000; font-weight:bold; text-decoration:none;'>$caption</a><br>";}
			if ($file!=$category){ $form.="<a href='#maslo=$caption' onclick='setMasloCategory(\"$file\");' style='margin-left:10px;'>$caption</a><br>";}
		}
		return $form;
	}
	function show_maslo_filters($category,$cols,$vals,$used_id){$db=new db;$form="";if ($category==""){$category=$this->getDefaultMasloCategory();}
		...
		for ($i=1;$i<=$n;$i++){
			$col_name=$db->result($r,$i-1,"col_name");array_push($colums,$col_name);
		}
	...
	function show_maslo_filter_items($category,$cols,$vals,$used_id,$colums){$db=new db;$form="";
		...
		if ($cols!=""){$checked=array();$cols2=array();
			foreach($cols as $col){if ($col!=""){array_push($cols2,$col);
				foreach($vals as $val){if ($val!=""){$vval=explode("=",$val);
					if ($val==($col."=".$vval[1])){array_push($checked,$vval[1]);}
				}}
			}}$cols=$cols2;
		}
		...
		foreach($colums as $colum){
			...
			for ($i=1;$i<=$n;$i++){
				...
				if ((!in_array($vl,$enable)) && $vl!=""){array_push($enable,$vl);$enable2.="$vl,";}
			}
		}
		...
		for ($j=1;$j<=$n1;$j++){
			...
			for ($i=0;$i<=$k-1;$i++){$value=$cn[$i];
				$link=" onclick='setCategoryFilter(this,\"$category\",\"$col_name\",\"$value\")'";
				$ich="";if (in_array($value,$checked)){$ich=" checked";}
...
//				if ($bonus>0)
	...
	function getActionBonusSumm($id,$client){$bonus=0;$bonus1=0;$odb=new odb;
		...
		while(odbc_fetch_row($r)){	$bonus1=odbc_result($r,"bonus");	}
		$bonus=$bonus-$bonus1;
		return $bonus;
	}

	function getRandomItem(){$odb=new odb; 
		//$r=$odb->query_td("select code from item order by rand() limit 1;");$code="";while(odbc_fetch_row($r)){$code=odbc_result($r,"code");}return $code;
		return "3397001543";
	}
	function getDefaultValuta(){$odb=new odb; 
		$r=$odb->query_td("select id from valuta where isCurrent='1' limit 1;");$valuta_id=0;
		while(odbc_fetch_row($r)){$valuta_id=odbc_result($r,"id");}
		return $valuta_id;
	}
	function getItemKurs($valuta_id){$odb=new odb;$slave=new slave;
		$r=$odb->query_td("select kurs from valuta where id='$valuta_id' limit 1;");$kurs=1;
		while(odbc_fetch_row($r)){$kurs=$slave->tomoney(odbc_result($r,"kurs"));}
		return $kurs;
	}
	function getItemCaption($item_id){$odb=new odb;  
		$r=$odb->query_td("select code,Name from item where id='$item_id' limit 1;");$caption="";
...
//		foreach()
		...
		$r=$odb->query_td("select id from item where code='$code' limit 1;");$id="";
	...
	function getItemIdFind($code){$odb=new odb;  
		...
		$r=$odb->query_td("select id from item where code='$code' or scode LIKE '$scode' limit 1;");$id="";
		while(odbc_fetch_row($r)){$id=odbc_result($r,"id");}
		return $id;
	}
	function getItemCaptionCode($item_id){$odb=new odb;  
		$r=$odb->query_td("select code,Name from item where id='$item_id' limit 1;");$caption="";$code="";
	...
	function getItemRemark($item_id){$odb=new odb;  
		$r=$odb->query_td("select help from item where id='$item_id' limit 1;");$remark="";
		while(odbc_fetch_row($r)){$remark=odbc_result($r,"help");}
		return $remark; 
	}
	function getItemProducentTd($item_id){$odb=new odb;  
		$r=$odb->query_td("select prod_id from item where id='$item_id' limit 1;");$prod_id="";$prod_td_id="";
		while(odbc_fetch_row($r)){$prod_id=odbc_result($r,"prod_id"); break;}
		$r=$odb->query_td("select tdid,name from producent where id='$prod_id' limit 1;");
	...
	function getKoef($discount_id){$odb=new odb; $slave=new slave;session_start(); $group_id=$_SESSION["client_group"];if ($group_id==""){$group_id=13;}
		...
		$r=$odb->query_td("select * from discounts where discount_id='$discount_id' and group_id='$group_id' limit 1;");
	...
	function getMDC($item_id,$kurs,$pricePro,$profit,$skid){$odb=new odb;$slave=new slave;session_start();$client_id=$_SESSION["client_id"];
		$r=$odb->query_td("select * from item where id='$item_id' limit 1;");$mdc=0;
		while(odbc_fetch_row($r)){
...
/*		Рассчёт фиксированной цены, Например Exist
			if ($client_id<>"") {
			$r=$odb->query_td("select Price from PriceListKlient where id='$item_id' and klient_id='$client_id' limit 1;");$PriceFix=0;
			while(odbc_fetch_row($r)){
...
*/		
	...
	function updateOrderItemPriceSumm($id,$item_id,$kol,$ExPrice,$exSumm){session_start(); $odb=new odb; $slave=new slave;
		$r=$odb->query_td("select * from item where id='$item_id' limit 1;");
		while(odbc_fetch_row($r)){
	...
	function getSkladName($id){session_start(); $odb=new odb;
		$name="";$r=$odb->query_td("SELECT Name,remark FROM subconto WHERE id = '$id' limit 1;");
...
//поиск товара 	
	function getArticleId($code,$item_id){$odb=new odb;$articleId=0;
		$r=$odb->query_td("select article_id,data from item_tecdoc where item_id='$item_id' limit 1;");
		while (odbc_fetch_row($r)){
...
//правило для GATES
			...
			try {
				...
				if (!$empty){
					...
					foreach ($result as $item){ $articleId=$item->articleId; $articleName=$item->articleName; $brandName=$item->brandName;}
				}
				if ($empty){
	    	    		$result = $soap->getArticleDirectSearchAllNumbers2(array(
            	  			'provider' => PROVIDER_ID,'lang' => 'ru','country' => 'ru',
	...
	function getArticleIdName($code,$item_id){$odb=new odb;$articleId=0;
		$r=$odb->query_td("select * from item_tecdoc where item_id='$item_id' limit 1;");
		while (odbc_fetch_row($r)){
			$articleId=odbc_result($r,"article_id");
			$articleData=odbc_result($r,"data");
...
//			if (substr($code,-3,3)==" st"){$code=str_replace(" st","",$code);}			
	...
	function showItemPhoto($item_id){session_start();$client=$_SESSION["client"];
		...
		if ($client!="" and $client!=0){$odb=new odb;
		...
		while(odbc_fetch_row($r)){
			...
			$istd=odbc_result($r,"istd");if ($istd==1){$td_ex=1;}
...
//		$td_ex=1;
	...
	function showItemInfo($item_id){session_start();$client=$_SESSION["client"];
		...
		if ($client!="" and $client!=0){$odb=new odb;
		$form_htm=RD."/tpl/catalogue_items_info.htm"; if (file_exists("$form_htm")){ $form = file_get_contents($form_htm);}
		list($caption,$code)=$this->getItemCaptionCode($item_id);
		...
		try {
			...
			$result=$result->data->array[0]->articleAttributes->array;$list="";$i=0;
	...
	function showAplicability($articleId){$odb=new odb;$list="";
		...
		if ($list==""){
			...
			try {
				...
				$result=$result->data->array; $LinkId=array();$TargetId=array();$i=0;
				foreach ($result as $item){$i+=1;
	...
	session_start(); $odb=new odb; $client=$_SESSION["client"];
		...
		if ($client!=0){$er=0;
			...
			// $query="select art from history_search where client='$client' order by id desc limit 1";
			$query="select art,prod_id,(days(current date)-days(data)) as ago from history_search where client='$client' and art='".strtolower($art)."' and prod_id='$by_producent' order by id desc limit 1";  //and (days(current date)-days(data))>0 
...
//				 print "FINDING!!!! $art_ses' и '$art' </n>'";							
	...
	session_start(); $odb=new odb; $client=$_SESSION["client"];$list="<div class='HistorySearchFrom'>";
		if ($client==""){
			...
			for ($i=1;$i<=$search_count;$i++){
				$art=$_SESSION["artSearch$i"];$art_cap=$art;
				if (strlen($art)>15){$art_cap=substr($art,0,13)."..";}
				$list.="<div><a href='#search=$art'>$art_cap</a></div>";
			}
		}
		if ($client!=""){
			$r=$odb->query_td("select art from history_search where client='$client' order by id desc limit 50");
			while(odbc_fetch_row($r)){
				$art=odbc_result($r,"art");$art_cap=$art;
				if (strlen($art)>15){$art_cap=substr($art,0,13)."..";}
				$list.="<div><a href='#search=$art'>$art_cap</a></div>";
			}
		}
	...
	function showStoProducent($producent=0){ $odb=new odb; $slave=new slave; 
		$menu.="<select name='sto_producent' id='sto_producent' size=1 class='tecFilter' onchange='loadStoProducentFilter(this[this.selectedIndex].value);'><option value='0'> --- выберите производителя ---  </option>";
		$r=$odb->query_td("select * from sto_producent where ison='1' order by id asc");
		while(odbc_fetch_row($r)){
			$id=odbc_result($r,"id");
			$name=odbc_result($r,"name");
	...
	function showStoCategory($producent=0,$category=0){ $odb=new odb; $slave=new slave;
		$menu.="<select name='sto_category' id='sto_category' size=1 class='tecFilter' onchange='loadStoCategoryFilter(\"$producent\",this[this.selectedIndex].value);'><option value='0'> --- выберите категорию оборудование ---  </option>";
		$r=$odb->query_td("select * from sto_category where producent='$producent' and ison='1' order by id asc");
		while(odbc_fetch_row($r)){
			$id=odbc_result($r,"id");
			$name=odbc_result($r,"name");
	...
	function showStoOtype($category=0,$otype=0){ $odb=new odb; $slave=new slave;
		$menu.="<select name='sto_otype' id='sto_otype' size=1 class='tecFilter' onchange='loadStoOtypeFilter(\"$category\",this[this.selectedIndex].value);'><option value='0'> --- выберите тип оборудование ---  </option>";
		if ($category!=0 and $category!=""){
			$r=$odb->query_td("select * from sto_otype where category='$category' and ison='1' order by id asc");
			while(odbc_fetch_row($r)){
				$id=odbc_result($r,"id");
	...
	function catalogue_maslo_page_navigation($category,$where,$page){ $db=new db;$slave=new slave;$kpp=100; if ($page==""){$page=$_GET["page"]; if ($page==""){$page="1";}}$cur_page=$page; $limit=" limit $kpp";
		if ($where!=""){
...
/*		if ($client=="" or $client==0){
	...
	function showMasloCaption($lider_id,$category){$db=new db;$caption="";
		$r=$db->query_lider("select caption from `items_$category` where `lider_id`='$lider_id' limit 1;");$n=$db->num_rows($r);
		if ($n==1){$caption=$db->result($r,0,"caption");}
		return $caption;
	}
	function getMasloDisplacement($lider_id,$category){$db=new db;$displacement="";
		$r=$db->query_lider("select displacement from `items_$category` where `lider_id`='$lider_id' limit 1;");$n=$db->num_rows($r);
	...
	function checkStoItemExist($code){ $odb=new odb; $ex=0;
		if ($code!=""){
			$r=$odb->query_td("select * from sto_items where code='$code' limit 1;");
...
while(odbc_fetch_row($r)){ $prm=0; $price1=""; $i++;
	...
	function showItemAnalog($item_id){session_start(); $odb=new odb; $slave=new slave; $dep="23";
		...
		$r=$odb->query_td("select * from item $where  $exclude order by code limit 100;");$kol=$n;$list="";$flist="";$kt=-1;$i=0;
		while (odbc_fetch_row($r)){ $prm=0; $price1="";$i++;
				$id=odbc_result($r,"id"); 
				$code=odbc_result($r,"code");
				$scode=odbc_result($r,"scode");
...
//						<td><a href='javascript:search_biart(\"$code\");'>$code</a></td>
		...
		$form=str_replace("{items_list}",$list,$form);
		$filter="по коду";if ($by_name=="1"){$filter="по названию";}if ($by_code=="1"){$filter.=" строгий отбор";}if ($by_sklad=="1"){$filter.=", только наличие";}
...
//							<td><a class='desc' href='javascript:search_biart(\"$code\");' style='text-decoration:none;'>$code</a></td>
	...
	function showItemAnalogLider($itemCode,$itemsArr){session_start(); $odb=new odb; $slave=new slave; $dep="23"; $exclude=" and prod_id not in (1134) ";
		$where="";foreach ($itemsArr as $item){$where.=" id='$item' or";}  if ($where!=""){$where=" where (".substr($where,0,-3).") $exclude";} 
		$r=$odb->query_td("select * from item $where limit 30;");$kol=$n;$list="";$i=0;$kt=-1;
		while(odbc_fetch_row($r)){ $prm=0; $price1=""; $i++; $icon_flag="";
			$id=odbc_result($r,"id"); 
			$code=odbc_result($r,"code");
			$scode=odbc_result($r,"scode");
...
";
		if ($cur_id!=$_SESSION["cur_id"]){$_SESSION["price_from"]="";$_SESSION["price_to"]="";$_SESSION["red_price"]="";$_SESSION["sale"]="";$_SESSION["best"]="";$_SESSION["special_offer"]="";}
		$price_from=$_SESSION["price_from"];$price_to=$_SESSION["price_to"];
		if ($_SESSION["red_price"]=="1"){$red_price_checked="checked=\"checked\"";}
		if ($_SESSION["sale"]=="1"){$sale_checked="checked=\"checked\"";}
		if ($_SESSION["best"]=="1"){$best_checked="checked=\"checked\"";}
		if ($_SESSION["special_offer"]=="1"){$special_offer_checked="checked=\"checked\"";}
		list($min_price,$max_price)=$this->get_min_max_price($cur_id);
		if ($price_from>$min_price){$price_from=$min_price; $_SESSION["price_from"]=$min_price;}
		if ($price_to<$max_price){$price_to=$max_price; $_SESSION["price_to"]=$max_price;}
		$form.="
		...
		$params=$_SESSION["params"];if (substr($params,-1)=="|"){$params=substr($params,0,-1);}
		$params=explode("|",$params);
		$r=$db->query("select * from catalogue_params where cat_id='$folder_id' order by lenta,id asc;");$n=$db->num_rows($r);
		for ($i=1;$i<=$n;$i++){
			...
			if ($param_type==2){
				$checked="";if (in_array($param_id, $params)){$checked="checked='checked'";}
			...
			if ($param_type==3){
				...
				if (in_array($param_id, $params)){
					...
					$FromCaption=$sub_params[0];$ToCaption=$sub_params[1];
				}
				$form=str_replace("{caption}",$param_caption,$form);
		...
		if ($cur_id!=$_SESSION["cur_id"]){$_SESSION["query"]="";}
		$form.="
		<div style='margin-left:10px;'>
	...
	function show_param_list($param_id,$type){session_start(); $db=new db;
		...
		$r=$db->query("select * from catalogue_sub_params where param_id='$param_id' order by lenta,id asc;");$n=$db->num_rows($r);$form="";
		for ($i=1;$i<=$n;$i++){
			$id=$db->result($r,$i-1,"id");
			$caption=$db->result($r,$i-1,"caption");
			$checked="";if (in_array($id, $sub_params)==true){$checked="checked='checked'";}
			if ($type==1){ $form.="<div><input type='checkbox' id='SubParam$id' value='1' $checked onclick='setFilter(\"$param_id\",\"$id\");'> $caption</div>";}
		}
		return $form;
	}
	function get_param_fromto($param_id){session_start(); $db=new db;
		$r=$db->query("select caption from catalogue_sub_params where param_id='$param_id' order by id asc limit 2;");$n=$db->num_rows($r);
	...
	function setFilter($param_id,$sub_param_id){session_start();$db=new db;$er=1;$ers=1;
		...
		if ($params!=""){
			foreach($params as $param){
				if ($param==$param_id){$er=0;
					if ($_SESSION["sub_params".$param_id]==""){$_SESSION["sub_params".$param_id]=$sub_param_id."|";}
					$sub_params=explode("|",$_SESSION["sub_params".$param_id]);
					if ($sub_params!=""){
						foreach($sub_params as $sub_param){ if ($sub_param==$sub_param_id){$ers=0; break;} }
						if ($ers==1){$_SESSION["sub_params".$param_id].=$sub_param_id."|";}
					}
					break;
				}
			}
			if ($er==1){$_SESSION["params"].=$param_id."|";$_SESSION["sub_params".$param_id]=$sub_param_id."|";if ($_SESSION["sub_params".$param_id]=="|"){$_SESSION["sub_params".$param_id]="";}}
		}
		return "ok";
	}
	function unsetFilter($param_id,$sub_param_id){
		...
		if ($params!=""){$new_params="";
			foreach($params as $param){
				if ($param!=$param_id){$new_params.=$param."|"; }
				if ($param==$param_id){ $kol=0;
					$sub_params=$_SESSION["sub_params".$param_id]; if (substr($sub_params,-1)=="|"){$sub_params=substr($sub_params,0,-1);}
					$sub_params=explode("|",$sub_params);
					if ($sub_params!=""){$new_sub_params="";
						foreach($sub_params as $sub_param){ if ($sub_param!=$sub_param_id and $sub_param!=""){ $new_sub_params.=$sub_param."|";$kol++;}}
						$_SESSION["sub_params".$param_id]=str_replace("||","|",$new_sub_params);
						if ($kol>0){$new_params.=$param."|";}
						if ($_SESSION["sub_params".$param_id]=="|" or $_SESSION["sub_params".$param_id]==""){unset($_SESSION["sub_params".$param_id]);}
					}
				}
			}
			$_SESSION["params"]=str_replace("||","|",$new_params);if ($_SESSION["params"]=="|"){unset($_SESSION["params"]);}
		}
		return "ok";
	}
	function setFilterFromTo($param_id,$from,$to){session_start();$db=new db;$er=1;$ers=1;
		...
		if ($params!=""){
			...
			if ($er==1){$_SESSION["params"].=$param_id."|";$_SESSION["sub_params".$param_id]=$from."|".$to;}
		}
		return "ok";
	}
	...
	function get_sklad_options($sklad){$db=new db; 
		$r=$db->query("select * from sklad where id='$sklad' limit 1;");$n=$db->num_rows($r);
		if ($n==1){
			$caption=$db->result($r,0,"caption");
	...
	function get_limit($top_id,$cur_id,$page,$where,$where_ses){session_start(); $db=new db;$slave=new slave; $kpp=15; if ($page==""){$page=$_GET["page"];}
		...
		if ($n==0){$r=$db->query("select c.id from catalogue c where c.ison='1' and c.is_folder='2' $where;");$n=$db->num_rows($r);}
		if ($n>=0){
			$kol=$n;
			if ($_SESSION["where_ses"]!=$where_ses){$page=""; $_SESSION["page"]=""; $_SESSION["where_ses"]=$where_ses;};
			$pg=$page; if ($pg==""){$pg="0";}
			$pg-=1;if ($pg<0){$pg=0;}
			$lmt=$kpp*$pg;
			if ($page=="all"){ $limit=" limit $kol"; }
...
//				$menu="<a class='navigation' href='#page=all' onClick='setRangePage(\"all\")'>".$LANG["all"]."</a> | ".$menu;
	...
	function get_model_img($model){$db=new db;
		...
		if ($n>0){
			$id=$db->result($r,0,"id");
			if (file_exists("uploads/images/catalogue/$model/$id.jpg")) { $file="$id.jpg";}
			if (!file_exists("uploads/images/catalogue/$model/$id.jpg")) { $file="nofoto.jpg";}
		}
		return $file;
	}
	function get_model_image($id){$db=new db;
		$r=$db->query("select cat from catalogue_galery where id='$id' limit 1;");$n=$db->num_rows($r);
		if ($n>0){
			$model=$db->result($r,0,"cat");
			$caption=$db->result($r,0,"caption");
			if (file_exists("uploads/images/catalogue/$model/$id.jpg")) { $file="<img src='thumb.php?image=catalogue/$model/$id.jpg&size=1000' border=0 alt='$caption' title='$caption'>";}
			if (!file_exists("uploads/images/catalogue/$model/$id.jpg")) { $file="<img src='thumb.php?image=catalogue/$model/nofoto.jpg&size=650' border=0 alt='$caption' title='$caption'>";}
		}
		return $file;
	}
	function show_catalogue_seo_info($cur_id){	$db=new db;
		$r=$db->query("select seo_info from catalogue where id='$cur_id' limit 1;");$n=$db->num_rows($r);
	...
	function show_catalogue_menu($top_id,$cur_id){ $db=new db; $slave=new slave;$dep="23"; $scroll_yes=$_GET["scroll"]; 
		...
		$r=$db->query("select id,caption from catalogue where (top_id='$top_id' and ison='1' and is_folder='1') order by $order;");$n=$db->num_rows($r);
		if ($n>0){
			for ($i=1;$i<=$n;$i++){
				$id=$db->result($r,$i-1,"id");
				$caption=$db->result($r,$i-1,"caption");
				$scroll="";if ($scroll_yes==""){$scroll="&scroll=minimize";}
				...
				if ($top_id=="0"){
					...
					if ($id==$cur_id){$menu.="<tr height='24' align='left'><td style='width:3px font-size:5px;' background='theme/images/mbs_l.png'></td><td background='theme/images/mbs.png' onclick='location.href=\"$url\";' style='cursor:pointer; background-repeat:repeat-x; '>&nbsp;<a href='$url' class='top_menu' style='color:#FFFFFF' style='color:#FFFFFF;'>$caption</a</td><td style='width:4px' background='theme/images/mbs_r.png'></td></tr><tr><td height='6'></td></tr>";}
					if ($id==$cur_id and $scroll_yes==""){ $menu.="<tr align='left'><td></td><td colspan=2>".$this->show_next_level_menu($cur_id)."</td></tr>"; }
				}
				if ($top_id!="0"){
					if ($id!=$cur_id){$menu.="<img src='theme/images/li2.png'> <a class='dep' href='$url'>$caption</a><div class='dm'></div>";}
					if ($id==$cur_id){$menu.="<img src='theme/images/li2_a.png'> <a class='dep' href='$url$scroll' style='color:#fb0100;'>$caption</a><div class='dm'></div>"; }
					if ($id==$cur_id and $scroll_yes==""){ $menu.=$this->show_next_level_menu($cur_id); }
				}
			}
	...
	function show_up_level_menu($cur_id,$catalogue_menu){$db=new db;$dep="23";
		...
		if ($n>0){
			...
			$top_main=""; $order="id asc";
			if ($top_id=="0"){ $order="id asc";}
			$r1=$db->query("select id,caption from catalogue where (top_id='$top_id' and is_folder='1' and ison='1') order by $order;");$n1=$db->num_rows($r1);
			for ($i=1;$i<=$n1;$i++){
				$id=$db->result($r1,$i-1,"id");
				$caption=$db->result($r1,$i-1,"caption");
				$url="?dep=$dep&top_id=$top_id&cur_id=$id";
				if ($top_id!="0"){
					if ($id==$cur_id){$up_menu.="<img src='theme/images/li2_a.png'> <a class='dep' href='$url'>$caption</a><div class='dm'></div>";}
					if ($id!=$cur_id){$up_menu.="<img src='theme/images/li2.png'> <a class='dep' href='$url'>$caption</a><div class='dm'></div>";}
				}
				if ($top_id=="0"){
	...
	function show_seo_tree(){ $db=new db;$dep="23";
		$r=$db->query("select id,short_caption,caption from catalogue where top_id='0' and ison='1' and is_folder='1' order by lenta,id asc limit 6;");$n=$db->num_rows($r);
		if ($n>0){ $tree="";
			for ($i=1;$i<=$n;$i++){
				$id=$db->result($r,$i-1,"id");
				$caption=$db->result($r,$i-1,"short_caption");if ($caption==""){ $caption=$db->result($r,$i-1,"caption");}
				$url="?dep=$dep&top_id=$top_id&cur_id=$id";
				$tree.="
					<div class='seo_tree'>
	...
	function show_model_rating($model){ $db=new db;$slave=new slave;
		...
		$r=$db->query("select votes from catalogue where id='$model' limit 1;");$n=$db->num_rows($r);
		if ($n==1){	$votes=$db->result($r,0,"votes");}
		if ($rate==0){$rating="0";}
		if ($rate>0 and $rate<=10){$rating="1";}
		if ($rate>10 and $rate<=20){$rating="2";}
		if ($rate>20 and $rate<=30){$rating="3";}
		if ($rate>30 and $rate<=40){$rating="4";}
		if ($rate>40){$rating="5";}
	...
	function save_model_vote($model,$rate){ $db=new db;$slave=new slave;session_start(); $client=$_SESSION["client"];
		...
		if ($client!=""){
			...
			if ($n==1){	$mes="Вы уже отдали свой голос за этот товар";}
			if ($n==0){
				$r1=$db->query("select votes from catalogue where id='$model';");$n1=$db->num_rows($r1);
				if ($n1=="1"){
	...
	function show_model_compare(){session_start(); $kolc=$_SESSION["kolc"]; $disc=$_SESSION["discount"];$db=new db; $slave=new slave; $dep="23"; $kours=new kours;
		...
		for ($i=1;$i<=$kolc;$i++){
			if ($_SESSION["model$i"]!=""){
				$r=$db->query("select * from catalogue where ison='1' and id='".$_SESSION["model$i"]."' limit 1;");$n=$db->num_rows($r);
				if ($n>0){ $prm=0;
					$id=$db->result($r,0,"id"); 
					$top_id=$db->result($r,0,"top_id");
					$caption=$db->result($r,0,"caption");
					...
					$price=$kours->show_cash_price($db->result($r,0,"price$disc"));
					$sale=$db->result($r,0,"sale");if ($sale=="1"){$prm="sale";}
					$rp=$db->result($r,0,"red_price");if ($rp!="" and $rp!="0"){$prm="rp";}
					$list.="<td>$model_list</td><td width='3px'></td>";
					$img_file=$this->get_model_img($id);$img="<img src='thumb.php?image=catalogue/$id/$img_file&size=150&prm=$prm'>";
