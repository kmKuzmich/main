	...
	function show_maslo_filter_items($category,$cols,$vals,$used_id,$colums){$db=new db;$form="";
		...
		for ($j=1;$j<=$n1;$j++){
			...
			for ($i=0;$i<=$k-1;$i++){$value=$cn[$i];
				...
				$ich="";if (in_array($value,$checked)){$ich=" checked='checked'";}
				$dis="";if (!in_array($value,$enable) && (count($cols)>1 || $j>1)){$dis=" disabled";$link=""; $stl=" style='color:#000;'";}
				$form.="<input type='checkbox' id='fil_$col_name' value='$value' $ich $dis $link> - <span $link $stl>$value</span><br>";
			}
...
/*		if ($client=="" or $client==0){
	...
	function showMasloCross($lider_id,$category){$db=new db;$form="";
		$r=$db->query_lider("select lider_id from `items_$category` itm INNER JOIN (SELECT `cross`,displacement from `items_$category` where `lider_id`='$lider_id') as crs ON crs.cross=itm.cross where  itm.cross!=0 group by itm.displacement;");$n=$db->num_rows($r);
		for ($i=1;$i<=$n;$i++){
	...
	function showMasloCrossBusket($lider_id,$category){$db=new db;$form="";
		$r=$db->query_lider("select lider_id from `items_$category` itm INNER JOIN (SELECT `cross` from `items_$category` where `lider_id`='$lider_id') as crs ON crs.cross=itm.cross where itm.lider_id!='$lider_id' and itm.cross!=0;");$n=$db->num_rows($r);
