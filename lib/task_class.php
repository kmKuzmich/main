<?php
class task {
	function show_task_calendar(){session_start();
		$form=$this->show_calendar("","");
		return $form;
	}
	function showTaskForm($data){session_start();$client=$_SESSION["client"]; $odb=new odb;$slave=new slave;if (strlen($data)==9){$data=substr($data,0,8)."0".substr($data,8,1);}
		$form_htm=RD."/tpl/task_form.htm";$form="";if (file_exists("$form_htm")){ $form = file_get_contents($form_htm);}
		...
		$r=$odb->query_td("select * from tasks where client='$client' and data='$data' order by id asc;");$k=0;
		...
		if ($list==""){ $list="<tr><td align='center'><br /><br /><h3>Список задач пуст</h3></td></tr>"; }
	...
	function newTask($data){session_start();$client=$_SESSION["client"]; $odb=new odb;$slave=new slave;
		if (strlen($data)==9){$data=substr($data,0,8)."0".substr($data,8,1);}
		$form_htm=RD."/tpl/task_item_form.htm";$form="";if (file_exists("$form_htm")){ $form = file_get_contents($form_htm);}
		$form=str_replace("{data_word}",$slave->data_word($data),$form);
		$form=str_replace("{data}",$data,$form);
	...
	function addTask($data,$caption,$data_end,$time_end,$email,$desc){session_start();$client=$_SESSION["client"]; $odb=new odb;$slave=new slave;
		if (strlen($data)==9){$data=substr($data,0,8)."0".substr($data,8,1);}
		if ($client!=""){ $caption=$slave->qq($caption);$desc=$slave->qq($desc);
		$odb->query_td("insert into tasks (client,caption,data,data_end,time_end,email,desc,ison) values ('$client','$caption','$data','$data_end','$time_end','$email','$desc','1');"); $answer="ok";}
		if ($client==""){$answer="Для создания задач необходимо авторизироваться!";}
		return $answer;
	}
	function show_calendar($m,$y){	$slave=new slave;
		$form_htm=RD."/tpl/date_form.htm";$form="";if (file_exists("$form_htm")){ $form = file_get_contents($form_htm);}$date_form="";
		if ((!$m) || (!$y)){ $m = date("m",time()); $y = date("Y",time()); }
		if ($day==""){$day=date("d",mktime(0));}
    	/*== get what weekday the first is on ==*/
	    $tmpd = getdate(mktime(0,0,0,$m,1,$y));
    	$month = $tmpd["month"]; 
	    $firstwday= $tmpd["wday"];if ($firstwday==0){$firstwday=6;}
		...
		if (($m-1)<1){ $m_p=12;} else {$m_p=$m-1;} if (($m-1)<1){ $y_p=$y-1;}else {$y_p=$y;}
		if(($m+1)>12){ $m_n=1;} else {$m_n=$m+1;} if (($m+1)>12){$y_n=$y+1;}else {$y_n=$y;}
		$d = 1; $wday = $firstwday; $firstweek = true;
		/*== loop through all the days of the month ==*/
		while ( $d <= $lastday) {
			/*== set up blank days for first week ==*/
	        if ($firstweek) {
    	        $date_form.="<tr align='center'>";
        	    for ($i=1; $i<=$firstwday-1; $i++) { $date_form.="<td class='text'><font size=2>&nbsp;</font></td>"; }
	            $firstweek = false;$wday= $tmpd["wday"];
    	    }
	        /*== Sunday start week with <tr> ==*/
    	    if ($wday==1) { $date_form.="<tr align='center' height='80'>"; }
        	/*== check for event ==*/  
			if (strlen($d)==1){$dd="0".$d;}else {$dd=$d;}
			$short_task_list=$this->showShortTaskList("$y-$m-$d");
			if ($d==$day){ $date_form.="<td class='calTDs' bgcolor='#faf7d7' onclick='showTaskForm(\"$y-$m-$d\");'><div class='calTDdate'>$d</div>".$short_task_list."</td>\n";}
			if ($d!=$day and $wday>0 and $wday<6){ $date_form.= "<td class='calTD' onclick='showTaskForm(\"$y-$m-$d\");'><div class='calTDdate'>$d</div>".$short_task_list."</td>\n"; }
			if ($d!=$day and ($wday==6 or $wday==0)){ $date_form.= "<td class='calTD' onclick='showTaskForm(\"$y-$m-$d\");'><div class='calTDdate'>$d</div>".$short_task_list."</td>\n"; }
    	    /*== Saturday end week with </tr> ==*/
        	if ($wday==6) { $wday=-1; }
			if ($wday==0) { $date_form.="</tr>"; }
	        $wday++;
//    	    $wday = $wday % 6;
        	$d++;
	    }
