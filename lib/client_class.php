...
//	               Просрочено " . $nearSumm . "грн.</span><br />";
//			}
            $form = str_replace("{client_name}", substr($name, 0, 40), $form);
...
//By Kuzichkin 11/06/2016 
	...
	{
//		if ($client_id == 4409) { //7562
        list($nearData, $nearDolg, $Dolg) = $this->getSubcontoNearDataSumNew($client_id);//5551//12515//14882
        $period = $nearData - date('Y-m-d');
        $period = -1;
        switch ($period) {
            case -1 :
                $mess = "Завтра $nearData наступит дата оплаты. Сумма платежа должна быть не менее $nearDolg";
                break;
            case 0:
                $mess = "Сегодня дата закончилась оплаты, но платежа на сумму $nearDolg от Вас мы ещё не получили, через 5 дней система заблокирует отгрузку товара";
                break;
            case 1:
                $mess = "Срок оплаты прошёл ещё вчера, но платежа на сумму $nearDolg нет. Для дальнейшей работы подтвердите просрочку";
                break;
            case 2:
                $mess = "Срок оплаты накладной, но платежа на сумму $nearDolg нет. Для дальнейшей работы подтвердите ";
                break;
        }

        echo "$period <br>";
//        if ( $nearData < date('Y-m-d')+17 ) {
//		    echo "<script type=\"text/javascript\">alert(\"Чего то там с задолженостью у клиента $client_id\");</script>";
//            echo "Сегодня ".date('Y-m-d')." для клиента $client_id  \n ближ дата :$nearData , Ближ сумма : $nearDolg ,\n  общий долг: $Dolg\n";
...
//        $name = substr($name,0,40);
    ...
    {
        session_start();
        if ($_SESSION["client"] != '' || $_SESSION["nearData"] == '' || $_SESSION["nearDolg"] == '' || $_SESSION["Dolg"] = '') {
            $odb = new odb;
            $slave = new slave;
            $nearData = "";
            $Dolg = 0;
            $r = $odb->query_td("SELECT sday, cDT,
                               CASE WHEN ( sday<cDT )  THEN GetDolg( id, cDT ) ELSE GetDolg( id,sday) END AS os,
                               GetDolg( id) as dolg
                              FROM (select K.id, CURRENT DATE AS cDT,
                                       GetDolgDate(K.id)+1 days as sday 
                                FROM klient K 
                                WHERE K.id='$client')                             
							  ");
            while (odbc_fetch_row($r)) {
            ...
            $Dolg = $_SESSION["Dolg"];
//            echo "<script>alert('$nearData')</script>";
        }
    ...
    function getSubcontoNearDataSum($client)
    {
        $odb = new odb;
        $slave = new slave;
        //Следующий запрос строит список документов долгов и просрочек,
        // непонятно зачем то вложенным запросом это сделано, может для DB2 потому что в SA он работает как обычно
        $r = $odb->query_td("
		select *
...
//			если аванс, то если аванс больше задолженности, то (сбросить дату документа), иначе прибавить к задолженности и если б.дата > даты документа то установить дату документа
            if (in_array($kinddoc_id, array(40, 61))) {
                if (($nearSumm < abs($os))) {
//					$nearSumm = 0; //обнулить задолженность-не надо, потому что потом ещё долги есть на которые этот аванс должен пойти,
                    $nearData = "";
                }
...
//
//
//			проверяем, если дата текущ.док.в строке больше чем текущая дата или вид документа аванс = изменить ближ.сум.опл.
