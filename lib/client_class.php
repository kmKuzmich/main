...
//			$form=str_replace("{nearSumm}", $nearSumm, $form);
			$form=str_replace("{nearSumm}", $saldo, $form);
//			$form=str_replace("{nearData}", $nearData, $form);
	...
	function getClientMessages(){$odb=new odb;$ex=0;session_start(); $client_id=$_SESSION["client"];
...
*/					
		...
		while(odbc_fetch_row($r)){$ex=1;
			$id=odbc_result($r,"id");
			$message="[#".odbc_result($r,"num")."] ".odbc_result($r,"header");
			$data=odbc_result($r,"day");
			$form_htm=RD."/tpl/client_messageBox_form.htm";if (file_exists("$form_htm")){ $form = file_get_contents($form_htm);}
	...
	function showMessageBox($mess_id){$odb=new odb;session_start(); $client_id=$_SESSION["client"];
//		$r=$odb->query_td("select M.id,M.Day,M.header,M.body,DS.name from Messages as M left outer join DocState DS on DS.id=M.state_id where M.id='$mess_id' and sDay<=current_date and (M.client_id='$client_id' or ('$client_id' in (select SubConto_id from SubContoTypes as TS where TS.SubcontoType_id=M.group_id) and not exists(select M1.id  from Messages as M1 where M1.id1=M.id and M1.client_id='$client_id'))) limit 0,1;");
		...
		while(odbc_fetch_row($r)){
			...
			$form_htm=RD."/tpl/client_messageBox_show.htm";if (file_exists("$form_htm")){ $form = file_get_contents($form_htm);}
...
//		print 'NEW id='.$mess_id.' KLIENT ='.$client_id;
	...
	function setMessageAnswer($mess_id,$answer){$odb=new odb;$odbA=new odb;session_start(); $client_id=$_SESSION["client"];
	...
	function getSubcontoNearDataSum($client){$odb=new odb;$slave=new slave;
	 ...
	 from DocOpen O
        ...
        join KindDoc KD on KD.id=D.KindDoc_id
		  ...
		  where \"os\"!=0 or \"vos\"!=0
		 order by \"sday\",\"num\";");
		 $nearData="";$nearSumm=0;$k=0;$i=0;$sDolgN=0;
		while(odbc_fetch_row($r))
		{$k+=1;$i+=1;
...
//			and ($os>0)
			){$nearSumm+=$os;$nearData=date("d-m-Y");}
