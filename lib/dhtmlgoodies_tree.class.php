...
class dhtmlgoodies_tree{
	...
	function writeJavascript(){
		...
		function expandNode(e,inputNode){
			...
			fleXenv.updateScrollBars();fleXenv.fleXcrollMain('PartsCroll');
		}
		function initTree(){
	...
	*/
	function addToArray($id,$name,$parentID,$url="",$target="", $hasChild=1, $onclick = ''){
		if ($hasChild==1){ $icon="/images/dhtmlgoodies_folder.gif"; $onclick=''; }
	...
	function drawTree(){
		...
		$this->mtree.="</ul>";
		$this->mtree.="</div><script>initTree();</script>"; 
		return $this->mtree;
	}
}
?>