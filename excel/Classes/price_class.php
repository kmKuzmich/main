...
class price {
	...
	function create_excel_file($data,$m){
		ini_set('include_path', ini_get('include_path').';../excel/Classes/');
		...
		header('Content-Disposition: attachment;filename="veles_price.xls"');
	...
	function create_menu(){
		...
		$r=$db->query("select id,code,caption from catalogue where top_id='0' and ison='1' and is_folder='1' order by code asc;");
		$n=$db->num_rows($r);
		if ($n>0){$m=0;
			for ($i=1;$i<=$n;$i++){$m++;
				...
				$data[$m]["id"]=$id;
				$data[$m]["code"]=$slave->qqback_in($code);
				$data[$m]["caption"]=$slave->qqback_in($caption);
	...
	function show_next_level($data,$cur_id,$m){
		...
		if ($n>0){
			for ($i=1;$i<=$n;$i++){$m++;
				$id=$db->result($r,$i-1,"id");
				$code=$db->result($r,$i-1,"code");
				$caption=$slave->qqback_in($db->result($r,$i-1,"caption"));
	...
	function show_models($data,$cur_id,$m){
		...
		$r=$db->query("select id,code,abr,caption,price$disc from catalogue where top_id='$cur_id' and is_folder='2' and ison='1' order by id asc;");
		$n=$db->num_rows($r);
		if ($n>0){
			for ($i=1;$i<=$n;$i++){$m++;
				$id=$db->result($r,$i-1,"id");
				$code=$db->result($r,$i-1,"code");
				$caption=$slave->qqback_in($db->result($r,$i-1,"caption"));
				$abr=$slave->qqback_in($db->result($r,$i-1,"abr"));
				$price=$slave->int_to_money($db->result($r,$i-1,"price$disc"));
				$data[$m]["id"]=$id;
				$data[$m]["code"]="=HYPERLINK(\"http://veles.km.ua/index.php?dep=$dep&w=show_model&top_id=$cur_id&model=$id\",\"".$code."\")";
				$data[$m]["caption"]=$caption." ".$abr;
				$data[$m]["price"]=$price;
			}
		}
		return array($data,$m);
