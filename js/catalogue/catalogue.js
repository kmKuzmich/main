...
function setAkbFilter(el, col_name, value, page) {
    startLoading();
    var cols = document.getElementById("akb_colname").value; cols = cols.split(',');
    var vals = document.getElementById("akb_value").value; vals = vals.split(',');
    var new_value = col_name + "=" + value;
    if (el) {
        if (el.checked) {
    ...
    document.getElementById("akb_colname").value = cols;
    document.getElementById("akb_value").value = vals;

    JsHttpRequest.query('content.php', {'w': 'showAkbRange','cols': cols,'vals': vals,'page': page},
...
function showAkbItemInfo(item_id) {startLoading();
    JsHttpRequest.query('content.php', {'w': 'showAkbItemInfo', 'item_id': item_id},
        function (result, errors) {
