function showBrandInfo(id){
	JsHttpRequest.query('content.php',{ 'w': 'showBrandInfo', 'id': id}, 
		function (result, errors){ if (errors) {alert(errors);} if (result){ 
			...
			fleXenv.updateScrollBars(); fleXenv.fleXcrollMain("BrandCroll");
	}}, true);	
}
function search_art(){
	...
	if (art.length>2){
		...
		var by_name=0; if (document.getElementById("by_name").checked){by_name=1;}
		var by_producent=document.getElementById("by_producent").value;
		
		JsHttpRequest.query('content.php',{ 'w': 'catalogue_art_find', 'art': art,'by_code':by_code,'by_sklad':by_sklad,'by_name':by_name,'by_producent':by_producent}, 
		function (result, errors){ if (errors) {alert(errors);stopLoading()} if (result){ 
			document.getElementById("range_list").innerHTML=result["content"];stopLoading();
			fleXenv.updateScrollBars(); fleXenv.fleXcrollMain("flexcroll");
		}}, true);
	}
...
function clearProd(){document.getElementById("by_producent").value="";}
function showItemActionRemark(item_id){startLoading();
	JsHttpRequest.query('content.php',{ 'w': 'showItemActionRemark', 'item_id': item_id}, 
	function (result, errors){ if (errors) {alert(errors);} if (result){ 
		showInfoForm();
		document.getElementById("InfoFormInfo").innerHTML=result["content"];
		document.getElementById("InfoForm").style.height="auto";
		document.getElementById("InfoForm").style.top="300px";stopLoading();
	}}, true);	
}
function showItemSklad(item_id){startLoading();
...
function showItemInfo(item_id){startLoading();
...
function showItemPhoto(item_id){startLoading();
...
function showItemAnalog(item_id){startLoading()
	...
	function (result, errors){ if (errors) {alert(errors);} if (result){ 
		...
		document.getElementById("wind_cont").innerHTML=result["content"];stopLoading();
		fleXenv.updateScrollBars(); fleXenv.fleXcrollMain("AnalogCroll");
	}}, true);	
}
...
function loadTecManufactureList(manufacture) {
	if (manufacture==0 || manufacture==''){manufacture=document.getElementById("manufacture")[document.getElementById("manufacture").selectedIndex].value;}
...
function loadTecModificationList(manufacture,model,modification) {
	if (manufacture==0 || manufacture==''){ manufacture=document.getElementById("manufacture")[document.getElementById("manufacture").selectedIndex].value;}
	if (model==0 || model==''){ model=document.getElementById("model")[document.getElementById("model").selectedIndex].value;}
	JsHttpRequest.query('content.php',{ 'w': 'loadTecModificationList', 'manufacture':manufacture, 'model':model,'modification':modification}, 
	function (result, errors){ if (errors) {alert(errors);} if (result){ 
...
function loadTecDocGroupsList(){
	...
	initTree();collapseAll();stopLoading();
	fleXenv.updateScrollBars();fleXenv.fleXcrollMain("InfoCroll");
} 
function loadTecGroupsList(manufacture,model,modification){
	startLoading();
	if (manufacture==0 || manufacture=='' || !manufacture){	manufacture=document.getElementById("manufacture")[document.getElementById("manufacture").selectedIndex].value;}
	if (model==0 || model=='' || !model){	model=document.getElementById("model")[document.getElementById("model").selectedIndex].value;}
	if (modification==0 || modification=='' || !modification){	modification=document.getElementById("modification")[document.getElementById("modification").selectedIndex].value;}
	JsHttpRequest.query('content.php',{ 'w': 'loadTecGroupsList', 'manufacture':manufacture, 'model':model, 'modification':modification}, 
		function (result, errors){ if (errors) {alert(errors);stopLoading();} if (result){ 
			document.getElementById("range_list").innerHTML=result["content"];stopLoading();
			...
			fleXenv.updateScrollBars();fleXenv.fleXcrollMain("InfoCroll");
			initTree();collapseAll();
			loadTecDetailsListMore(manufacture,model,modification,vr[0],opbrd);
	}}, true);stopLoading();
}
function loadTecDetailsList(manufacture,model,modification,groups,brandNo) {startLoading();
	...
	if (document.getElementById("PartsCroll")){
	JsHttpRequest.query('content.php',{ 'w': 'loadTecDetailsList', 'manufacture':manufacture, 'model':model, 'modification':modification, 'groups':groups, 'brand':brandNo}, 
	function (result, errors){ if (errors) {alert(errors);stopLoading();} if (result){ 
		document.getElementById("PartsCrolls").innerHTML=result["content"];
		fleXenv.updateScrollBars(); fleXenv.fleXcrollMain("PartsCroll");stopLoading();
	}}, true);
	}
}
function loadTecDetailsListMore(manufacture,model,modification,groups,brandNo) {startLoading();
	var hash=location.hash;
	var op=hash.split("=");
	if (op[0]!="#groups"){stopLoading(); }
	if (op[0]=="#groups"){ 
		var vr=op[1].split("/");
		var opbrd="";
		if (!vr[4]){opbrd="";}if (vr[4]){opbrd=vr[4];}
		JsHttpRequest.query('content.php',{ 'w': 'loadTecDetailsList', 'manufacture':manufacture, 'model':model, 'modification':modification, 'groups':vr[0], 'brand':opbrd}, 
		function (result, errors){ if (errors) {alert(errors);stopLoading();} if (result){ 
			document.getElementById("PartsCrolls").innerHTML=result["content"];
			fleXenv.updateScrollBars(); fleXenv.fleXcrollMain("PartsCroll");stopLoading();
		}}, true);
...
function loadStoProducentFilter(producent) {
	loadStoCategoryF(producent,0);loadStoOtypeF(0,0); loadStoItemsFilter();
}
function loadStoCategoryFilter(producent,category) {
	loadStoCategoryF(producent,category);loadStoOtypeF(category,0); loadStoItemsFilter();
}
function loadStoOtypeFilter(category,otype) {
	loadStoOtypeF(category,otype); loadStoItemsFilter();
}
...
function loadStoItemsFilter() {
	startLoading();
	var producent=0; if (document.getElementById("sto_producent").selectedIndex){ producent=document.getElementById("sto_producent")[document.getElementById("sto_producent").selectedIndex].value; }
	var category=0; if (document.getElementById("sto_category").selectedIndex){ category=document.getElementById("sto_category")[document.getElementById("sto_category").selectedIndex].value; }
	var otype=0; if (document.getElementById("sto_otype").selectedIndex){ otype=document.getElementById("sto_otype")[document.getElementById("sto_otype").selectedIndex].value; }
	JsHttpRequest.query('content.php',{ 'w': 'loadStoItemsFilter', 'producent':producent, 'category':category, 'otype':otype }, 
	function (result, errors){ if (errors) {alert(errors);} if (result){ 
		document.getElementById("items_list").innerHTML=result["content"];stopLoading();
	}}, true);
}
...
function showAplicability(articleId){startLoading();
...
function showActionInfo(url){startLoading();
	JsHttpRequest.query('content.php',{ 'w': 'showActionInfo', 'url': url}, 
	function (result, errors){ if (errors) {alert(errors);} if (result){ 
		showInfoForm();
		document.getElementById("InfoFormInfo").innerHTML=result["content"];
		document.getElementById("InfoForm").style.height="auto";
		document.getElementById("InfoForm").style.top="200px";stopLoading();
		document.getElementById("InfoForm").style.zIndex="999999";
	}}, true);	
}
function setMasloCategory(category){startLoading();
...
function setMasloCategoryFilters(category){startLoading();
...
function setCategoryFilter(el,category,col_name,value,page){startLoading();
	var cols=document.getElementById("maslo_colname").value; cols=cols.split(',');
	var vals=document.getElementById("maslo_value").value; vals=vals.split(',');
	var new_value=col_name+"="+value;
	if (el){
		if (el.checked){
			if (cols.indexOf(col_name) > -1){}else{cols.push(col_name);}
			if (vals.indexOf(new_value) > -1){}else{vals.push(new_value);}
		}else{
	...
	JsHttpRequest.query('content.php',{ 'w': 'showMasloRange', 'category': category, 'cols':cols, 'vals':vals, 'page':page}, 
	function (result, errors){ if (errors) {} if (result){ 
...
function showMasloItemInfo(item_id,category){startLoading();
