...
function SaveRegistration() {
    ...
    if (submit_email == 0) {
        document.getElementById("RegError").innerHTML += "Проблема с указанным E-mail<br>";
    }
    if (submit_name == 0) {
    ...
    if (submit_email == 0) {
        document.getElementById("RegError").innerHTML += "Email <" + email + "> введён некорректно, убедитесь что он не содержит скрытых (запрещённых) символов (попробуйте указать его вручную, а не через вставку и введён до конца наприрмер: mail@ukr.net)<br>";
    }
