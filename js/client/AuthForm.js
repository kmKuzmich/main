function showForgetForm() {
	...
	document.getElementById("ForgetForm").style.top="200px";showAlBg();
}
function closeForgetForm() {
	...
	document.getElementById("ForgetError").innerHTML="";closeAlBg();
}
function showForgetAnswerForm() {
	...
	document.getElementById("ForgetAnswerForm").style.top="200px";showAlBg();
}
function closeForgetAnswerForm() {
	...
	document.getElementById("ForgetAnswerForm").style.top="0%";closeAlBg();
}
...
function closeRegistrationForm() {
	...
	document.getElementById("RegistrationForm").style.top="0%";closeAlBg();
}
function showRegistrationAnswerForm(answer) {
	...
	document.getElementById("RegistrationAnswer").innerHTML=answer;showAlBg();
}
function closeRegistrationAnswerForm() {
	...
	document.getElementById("RegistrationAnswer").innerHTML="";closeAlBg();
}
function SaveRegistration(){
	...
	if (submit_email==0){ document.getElementById("RegError").innerHTML+="Введите Ваш E-mail<br>"; }
	if (submit_name==0){ document.getElementById("RegError").innerHTML+="Укажите Ваше Имя<br>";}
	if (submit_address==0){ document.getElementById("RegError").innerHTML+="Укажите по какому адресу Вы проживаете<br>";}
	if (submit_phone==0){ document.getElementById("RegError").innerHTML+="Введите Ваш номер телефона<br>";}
	if (submit_email==0){ document.getElementById("RegError").innerHTML+="Email принадлежит другому пользователю<br>";}
	if (submit_email==1 && submit_name==1 && submit_address==1 && submit_phone==1){
		JsHttpRequest.query('content.php',{ 'w': 'SaveRegistration', 'recaptcha_challenge_field':recaptcha_challenge_field, 'recaptcha_response_field':recaptcha_response_field,'email': email, 'name': name, 'state':state, 'city': city, 'new_city':new_city, 'address': address, 'phone': phone,'activity':activity}, 
		function (result, errors){  if (errors) {alert(errors);} if (result){ 
			if (result["err"]=="0"){
				closeRegistrationForm();
...
function AuthAcount(){ startLoading();
	var reg = /^([A-Za-z0-9_\-\.])+\@([A-Za-z0-9_\-\.])+\.([A-Za-z]{2,4})$/;
	...
	if (submit_email==0){
		document.getElementById("AuthError").innerHTML="Введите Ваш E-mail";stopLoading();
	}
	if (submit_pass==0){
		document.getElementById("AuthError").innerHTML="Введите Ваш пароль";stopLoading();
	}
...
function checkPhone(place){
	check_nom(place);
	var ph=place.value;
	if (ph.length<=3){place.value="380";}
	if (ph.length>12){place.value=place.value.substring(0,12);}
}
